---
exam:
  title: 江苏省普通高校“专转本”选拔考试 计算机基础综合正式模拟试卷
  subtitle: "科目代码: 204（正式考试难度）"
  confidential_label: 机密★启用前
  duration_minutes: 120
  total_score: 150
  variant: official
instructions:
  - 本卷分为客观题和主观题两部分，请按题号顺序在指定答题区域内作答。
  - 单项选择题只有一个最符合题意的选项；多项选择题至少有两个正确选项，错选不得分。
  - 简答题、计算题、编程题和综合题须写出必要过程、关键结论或核心代码。
---
...
A. 解释程序一定先生成目标代码再执行
B. 编译程序不需要进行词法分析
C. 编译方式通常更有利于重复执行同一程序
D. 解释方式生成的可执行文件通常独立于解释器
...
B. 数据只能以十进制存储
C. 指令和数据统一存放在存储器中
D. 计算机由运算器、控制器、存储器、输入和输出设备等组成

## 22
@answer: ABD
关于进程调度与死锁的说法，正确的有（ ）。
A. 抢占式调度可提升系统响应能力
B. 死锁产生通常与互斥、请求并保持等条件有关
C. 只要有资源竞争就一定发生死锁
D. 银行家算法是一种避免死锁的思路
...
A. `GROUP BY` 常与聚合函数联合使用
B. `WHERE` 可以直接筛选聚合后的分组结果
C. `ORDER BY` 用于指定结果排序方式
D. `COUNT(*)` 可统计结果集行数
...
@answer: ABC
下列关于常见安全措施的说法，正确的有（ ）。
A. 强口令策略有助于降低账户被暴力破解风险
B. 最小权限原则可减少误操作或越权访问影响
C. 数据备份有助于应对勒索软件等破坏事件
D. 关闭日志记录通常能提高系统安全性
...
@answer_lines: 5
@keywords: 虚拟存储, 分页
@analysis: 应说明分页式虚拟存储通过页表把逻辑地址映射为物理地址，并指出缺页时需要进行页面置换。
简述分页式虚拟存储管理的基本思想，并说明发生缺页时系统通常要做什么。

## 52
@answer_lines: 5
@keywords: 数据库范式, 冗余
@analysis: 应围绕减少冗余、避免更新异常、提高一致性展开，可举出插入、删除或修改异常。
为什么关系数据库设计中要进行规范化？请结合至少一种异常现象说明。

## 53
@answer_lines: 5
@keywords: 面向对象, 封装, 继承, 多态
@analysis: 应分别说明三者的含义及其对复用、维护和接口设计的作用。
简述面向对象程序设计中封装、继承、多态三者各自的作用。

## 54
@answer_lines: 5
@keywords: 网络安全, 身份认证, 访问控制
@analysis: 可从身份可信、权限最小化、行为可审计等角度回答。
...
@answer_lines: 8
@keywords: 进制转换
@analysis: `(101101.101)_2 = 45.625_10 = (55.5)_8 = (2D.A)_{16}`，应写出整数部分与小数部分转换过程。
将二进制数 `(101101.101)_2` 分别转换为十进制、八进制和十六进制。

## 56
@answer_lines: 8
@keywords: 时间复杂度, 循环分析
@analysis: 外层循环执行 `n` 次，内层分别执行 `1+2+...+(n-1)` 次，总复杂度为 `O(n^2)`。
...
@answer_lines: 10
@keywords: 字符串, 哈希, 计数
@analysis: 可使用哈希表统计字符频次，再按出现顺序或字典序输出；重点是正确统计与边界处理。
...
## 58
@keywords: 网络, 数据库, 程序设计, 系统分析
某高校拟开发“实验室预约管理系统”。系统需要支持学生登录、查看空闲实验室、提交预约申请、教师审批、系统记录预约日志，并在实验室设备冲突时拒绝重复预约。

### 58.1
@score: 6
@answer_lines: 4
@analysis: 至少应提到用户表、实验室表、预约表、审批/日志相关表中的三类，并说明主键或关联关系。
从数据库设计角度，至少给出该系统应包含的 3 个核心数据表，并说明它们各自记录的关键内容。

### 58.2
@score: 6
@answer_lines: 4
@analysis: 可从身份认证、权限控制、日志审计、输入校验、并发控制等方面作答。
从系统安全与正确性角度，写出该系统在实现时必须考虑的 3 项关键措施，并说明原因。

### 58.3
@score: 8
@answer_lines: 6
@analysis: 应体现“查询可用资源 -> 校验时间冲突 -> 写入申请 -> 记录日志/等待审批”的流程，若考虑事务控制更佳。
